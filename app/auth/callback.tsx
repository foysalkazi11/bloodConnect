...
import { Heart, CircleCheck as CheckCircle, CircleAlert as AlertCircle, RefreshCw } from 'lucide-react-native';
...
export default function AuthCallbackScreen() {
  ...
  const handleAuthCallback = async () => {
    ...
    try {
      ...
      console.log('AuthCallback: URL analysis:', { error, errorCode, errorDescription });
      ...
      if (error) {
        ...
        if (error === 'access_denied' && (errorCode === 'otp_expired' || errorDescription?.includes('expired'))) {
          setStatus('error');
          setMessage('Your email verification link has expired. Please request a new verification email.');
          setCanRetry(true);
          
          showNotification({
            type: 'warning',
            title: 'Link Expired',
            message: 'Your verification link has expired. You can request a new one.',
            duration: 6000,
          });
          return;
        } else {
      ...
      console.log('AuthCallback: No errors in URL, proceeding with verification...');
      ...
      if (result.success) {
        ...
        showNotification({
          ...
          message: 'Your account has been activated. Please complete your profile to start using BloodConnect.',
          duration: 4000,
        });
