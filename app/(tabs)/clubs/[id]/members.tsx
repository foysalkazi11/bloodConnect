...
import { ArrowLeft, Search, UserPlus, Crown, Shield, User, MoveVertical as MoreVertical, MessageCircle, Phone, Bell } from 'lucide-react-native';
...
export default function ClubMembersScreen() {
  ...
  const loadMembers = async () => {
    try {
      ...
      const formattedMembers: ClubMember[] = data.map(member => ({
        ...
        last_active: new Date(Date.now() - Math.random() * 24 * 60 * 60 * 1000).toISOString(), // Random for demo
  ...
  const getRoleIcon = (role: string) => {
    switch (role) {
      ...
      case 'moderator': return <Shield size={16} color="#3B82F6" />;
            ...
            {selectedMember && (
              <>
                <View style={styles.memberModalHeader}>
                  ...
                  <Text style={styles.memberModalName}>{selectedMember.name}</Text>
                  <Text style={styles.memberModalRole}>{selectedMember.role}</Text>
                  {selectedMember.blood_group && (
                    <View style={styles.modalBloodGroup}>
                      <Text style={styles.modalBloodGroupText}>{selectedMember.blood_group}</Text>
                    </View>
                  )}
                </View>
                    ...
                    <>
                      {selectedMember.role === 'member' && (
                        ...
                        >
                          ...
                          <Text style={styles.memberActionText}>Promote to Moderator</Text>
                        </TouchableOpacity>
                      )}
                      ...
                      {selectedMember.role !== 'admin' && (
                        ...
                        >
                          ...
                          <Text style={[styles.memberActionText, styles.dangerActionText]}>Remove from Club</Text>
                        </TouchableOpacity>
                      )}
                    </>
                  )}
